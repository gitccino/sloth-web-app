...
const sidebarMenuButtonVariants = cva(
  ...
  [&>span:last-child]:truncate [&>svg]:size-5 [&>svg]:md:size-4 [&>svg]:shrink-0',
  {
    variants: {
      variant: {
        default:
          'hover:bg-sloth-aside-background-hover hover:text-sloth-foreground',
        outline:
...
function SidebarMenuAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-sidebar-foreground ring-sidebar-ring peer-hover/menu-button:text-sidebar-accent-foreground absolute top-1/2 -translate-y-1/2 right-1 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-hidden transition-transform focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
        // Increases the hit area of the button on mobile.
        'after:absolute after:-inset-2 md:after:hidden',
        'group-data-[collapsible=icon]:hidden',
        showOnHover &&
